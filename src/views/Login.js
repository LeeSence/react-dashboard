...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response) {
        console.error("response.Data에러", error.response.data);
        console.error("response.status에러", error.response.status);
        console.error("response.headers에러", error.response.headers);
        ...
        >
          <Col className="px-xl-2 mx-auto" sm="8" md="6" lg="12">
            <CardTitle tag="h2" className="fw-bold mb-1">
              MetalCycle에 오신걸 환영합니다! 👋
            </CardTitle>
            <CardText className="mb-2">
              키오스크를 이용하실려면 이메일과 비밀번호를 입력해주세요
            </CardText>
            {/* <Form
